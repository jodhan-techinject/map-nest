<script>
    ...
    import {enableDarkMode, enableLightMode} from "$lib/common/utils/localStorageController";
</script>
...
<header class="h-[4rem] shadow-sm shadow-primary-200  {$darkMode ? 'bg-black' : ' bg-primary-100'}">
    <div class="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav class="flex gap-7">
            {#if $darkMode}
                <button on:click={()=>{enableLightMode()}} aria-label="github-link">
            ...
            {:else}
                <button on:click={()=>{enableDarkMode()}} aria-label="github-link">
